    ...
    <div className={`space-y-6 ${isDark ? 'text-slate-100' : 'text-slate-800'}`} id="student-dashboard-root">
      ...
      <div className="grid md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 space-y-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-205 shadow-sm" id="student-notifications">
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
              <h3 className="text-xs sm:text-sm font-extrabold text-slate-900 flex items-center gap-2 uppercase tracking-wide">
                <Bell className="w-4 h-4 text-emerald-600" /> Notification Feed
              </h3>
              {notifications.filter(n => !n.isRead).length > 0 && (
                <span className="bg-rose-500 text-white text-[9.5px] font-black px-1.5 py-0.5 rounded-full font-mono shrink-0">
            ...
            <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
              {notifications.length > 0 ? (
                notifications.map((n) => (
                  <div 
                    key={n.id} 
                    className={`p-3 rounded-xl border transition-all text-xs text-left relative flex items-start gap-2.5 ${
                      n.isRead 
                  ...
                  >
                    <div className="mt-0.5">
                      {n.type === 'upload' && <Play className="w-4.5 h-4.5 text-indigo-500 bg-indigo-50 p-1 rounded-md border border-indigo-105 shrink-0" />}
                      {n.type === 'update' && <Sparkles className="w-4.5 h-4.5 text-amber-500 bg-amber-50 p-1 rounded-md border border-amber-105 shrink-0" />}
                      {n.type === 'unlock' && <CheckCircle className="w-4.5 h-4.5 text-emerald-500 bg-emerald-50 p-1 rounded-md border border-emerald-105 shrink-0" />}
                      {n.type === 'assign' && <BookOpen className="w-4.5 h-4.5 text-teal-500 bg-teal-50 p-1 rounded-md border border-teal-105 shrink-0" />}
                    </div>

                    <div className="flex-grow min-w-0">
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="font-extrabold text-slate-900 text-xs truncate max-w-[130px]" title={n.title}>{n.title}</h4>
                        <span className="text-[9px] font-mono text-slate-400 font-bold shrink-0">{n.timestamp}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        {n.type === 'upload' && (
                          <button
                            onClick={() => onNavigateTo('videos')}
                            className="text-[9px] font-black text-indigo-650 hover:underline flex items-center gap-0.5 bg-indigo-50 border border-indigo-100 rounded px-1.5 py-0.5 cursor-pointer"
                          >
                            Watch Lecture <ChevronRight className="w-2.5 h-2.5" />
                          </button>
                        )}
                        {n.type === 'unlock' && n.message.toLowerCase().includes('quiz') && (
                          <button
                            onClick={() => onNavigateTo('assessments')}
                            className="text-[9px] font-black text-emerald-700 hover:underline flex items-center gap-0.5 bg-emerald-50 border border-emerald-100 rounded px-1.5 py-0.5 cursor-pointer"
                          >
                            Take Quiz <ChevronRight className="w-2.5 h-2.5" />
                          </button>
                        )}
                        {!n.isRead && onMarkNotificationRead && (
                          <button
                            onClick={() => onMarkNotificationRead(n.id)}
                            className="text-[9.5px] text-slate-400 hover:text-emerald-600 font-bold ml-auto flex items-center gap-0.5 cursor-pointer"
                            title="Mark Read"
                          >
                ...
                <div className="p-4 border border-dashed rounded-xl text-center text-slate-400 text-[11px] italic">
                  No notifications yet. Check back later for course updates.
                </div>
              )}
            </div>
          </div>
